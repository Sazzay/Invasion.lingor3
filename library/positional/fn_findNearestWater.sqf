/*
	...
	Description:
	Finds the nearest body of water in a specified radius with specified min & max depth.
...
for "_i" from ((_pos select 0) - _radius) to ((_pos select 0) + _radius) step 100 do {
	for "_i2" from ((_pos select 1) - _radius) to ((_pos select 1) + _radius) step 100 do {
		if ((surfaceIsWater [_i, _i2]) and (abs(getTerrainHeightASL [_i, _i2]) > _mind) and (abs(getTerrainHeightASL [_i, _i2]) < _maxd)) then {
