		...
		[
			...
			{},	// Code executed on every progress tick						
			{
				...
				if (_nearestTents isEqualTo []) then {
								...
								[_crateObject,_tentObject],
								{
									[
										...
										{},	// Code executed on every progress tick						
										{
											...
											if ((["ace_main"] call I_fnc_isAddonActive) isEqualTo true) then {
												_tentObject setVariable ["ACE_isMedicalFacility",false,true];
											};
					...
					["Unable to place Medical Tent. You must be at least 20 meters or more away from another tent."] call I_fnc_notification;
				};
			}, // Code executed on completion
			{},	// Code executed on interrupted
			[(_this select 0)],
