/*
	...
	Notes: 0.65~ execution time maximum.
*/

[[], []] params ["_array", "_groups"];
	...
	if (((missionNamespace getVariable "RESTRICT_ACS_SEA") isEqualTo false) and !(_array isEqualTo [])) then {
		{
			if ([(getPos _x), (getDir _x), 300] call I_fnc_findEmptyWaterLine) then {
				_groups pushBack ([(selectRandom I_DEF_LAND_AMBUSH_GROUPS), selectRandom ([_x] call I_fnc_findAmbushLocation)] call I_fnc_createGroup);
