...
[{
...

	{
		...
		if ((count _players >= 1) and ((_var select 2) < 21)) then {
			[format ["ADS_TOWN_%1", [_x select 0, _x select 1]], [(_var select 0), (_var select 1), (_var select 2) + 1 + ((count _players) / 10), false]] call I_fnc_setVariable;
		};
	} forEach _pos;
