...
while {true} do {
	...
	{
		if ((typeOf _x) in ["rhs_prv13", "rhs_p37", "RU_WarfareBAntiAirRadar"] and (alive _x)) then {
			_radar = _radar + 1;
		};
		if ((typeOf _x) in ["RU_WarfareBBarracks", "RU_WarfareBHeavyFactory"] and (alive _x)) then {
			_reinforcement = _reinforcement + 1;
		};
		if ((typeOf _x) in ["RU_WarfareBLightFactory", "RU_WarfareBUAVterminal", "RU_WarfareBArtilleryRadar"] and (alive _x)) then {
			_support = _support + 1;
		};
	} forEach _array;
